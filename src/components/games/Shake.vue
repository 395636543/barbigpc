...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      httpData: function () {
        var configdata;
        this.$http.get('./src/api/shakedata.json').then(function (res) {
          //请求成功函数
          configdata = JSON.parse(res['bodyText'])
          console.log(JSON.parse(res['bodyText']))
          this.barname = configdata.data.barname
          this.count = configdata.data.count
          this.videourl = configdata.data.videourl
        }, function () {
          //请求失败函数
        })
      },
